...
module.exports = grammar(add_inline_rules({
    ...
    rules: {
        ...
        full_reference_link: $ => prec.dynamic(2 * PRECEDENCE_LEVEL_LINK, prec.right(seq(
            $._link_text,
            $.link_label,
            optional($._qmd_attribute)
        ))),
        collapsed_reference_link: $ => prec.dynamic(PRECEDENCE_LEVEL_LINK, prec.right(seq(
            $._link_text,
            '[',
            ']',
           optional($._qmd_attribute)
        ...
        _image_inline_link: $ => prec.dynamic(PRECEDENCE_LEVEL_LINK, prec.right(seq(
            $._image_description,
            '(',
            repeat(choice($._whitespace, $._soft_line_break)),
            optional(seq(
        ...
        _image_shortcut_link: $ => prec.dynamic(3 * PRECEDENCE_LEVEL_LINK, prec.right(seq($._image_description_non_empty, 
          optional($._qmd_attribute)
        ))),
        _image_full_reference_link: $ => prec.dynamic(PRECEDENCE_LEVEL_LINK, prec.right(seq($._image_description, $.link_label, 
          optional($._qmd_attribute)
        ))),
        _image_collapsed_reference_link: $ => prec.dynamic(PRECEDENCE_LEVEL_LINK, prec.right(seq($._image_description, '[', ']', 
          optional($._qmd_attribute)
        ))),
